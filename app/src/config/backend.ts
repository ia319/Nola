...
export function getApiBaseUrl(environment?: RuntimeEnvironment): string {
  if (!environment) {
...
export function getRealtimeWebSocketBaseUrl(environment?: RuntimeEnvironment): string {
  if (!environment) {
    return getActiveWebSocketBaseUrl()
  }
