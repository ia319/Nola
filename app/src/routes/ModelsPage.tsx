...
export function ModelsPage() {
  ...
  // Build seed map from REST snapshot so in-flight downloads survive page reload.
  const initialDownloads = useMemo(() => {
    const map = new Map<string, DownloadState>()
    for (const m of models) {
      if (m.download_progress) {
        map.set(m.model_id, toDownloadState(m.download_progress))
      }
    }
    return map
  }, [models])
