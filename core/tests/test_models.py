...
class TestTaskDatabase:
    ...
    def test_update_status_respects_terminal_state(self, test_db):
        """update_status() should not overwrite terminal states."""
        file_db, task_db = test_db
        ...
        # Complete the task (terminal state)
    ...
    def test_update_status_cancelled_task(self, test_db):
        """update_status() should not overwrite cancelled status."""
        file_db, task_db = test_db
        ...
        assert result is False
        task = task_db.get_task("task-001")
        assert task["status"] == "cancelled"  # Unchanged
        assert task["error"] is None  # Error not set
